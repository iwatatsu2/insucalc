...
const STAPLE_OPTIONS = [
  ...
  },
  {
    ...
    choices: [
      ...
      { label: '6枚切2枚',  grams: 120 },
      { label: 'もち1個',   grams: 50  },
      { label: 'もち2個',   grams: 100 },
    ],
  },
  {
    key: 'noodle',
...
export default function FoodSelector({ onChange }: Props) {
  ...
  const [selectedStaples, setSelectedStaples] = useState<SelectedStaple[]>([]);
  ...
  const stapleCarbs = selectedStaples.reduce((sum, s) => sum + s.carbs, 0);
  const sideCarbs   = selectedSides.reduce((sum, f) => sum + f.carbs, 0);
  const totalCarbs  = stapleCarbs + sideCarbs;

  const notify = (staples: SelectedStaple[], sides: FoodItem[]) => {
    const sc = staples.reduce((sum, s) => sum + s.carbs, 0);
    const sd = sides.reduce((sum, f) => sum + f.carbs, 0);
    const summary = [
      ...staples.map(s => `${s.emoji}${s.label}（${s.choiceLabel}）`),
      ...sides.map(f => f.name),
    ];
    onChange(sc + sd, summary);
  };

  const selectStaple = (opt: typeof STAPLE_OPTIONS[number], choice: { label: string; grams: number }) => {
    const carbs = Math.round(choice.grams * opt.ratio);
    const item: SelectedStaple = {
      key: opt.key, label: opt.label, emoji: opt.emoji,
      grams: choice.grams, carbs, choiceLabel: choice.label,
    };
    // 同じ種類があれば置き換え
    const next = selectedStaples.filter(s => s.key !== opt.key);
    next.push(item);
  ...
  const removeStaple = (key: StapleKey) => {
    const next = selectedStaples.filter(s => s.key !== key);
    setSelectedStaples(next);
    notify(next, selectedSides);
  };
      ...
      {tab === 'staple' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {selectedStaples.length > 0 && (
            <div style={{ padding: '10px 14px', background: '#EFF6FF', borderRadius: 14, border: '1.5px solid #BFDBFE' }}>
              <div style={{ fontSize: 11, color: '#3B82F6', fontWeight: 700, marginBottom: 6 }}>選択中の主食</div>
              {selectedStaples.map(s => (
                <div key={s.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 0' }}>
                  <span style={{ fontSize: 13, color: '#1E40AF' }}>{s.emoji} {s.label}（{s.choiceLabel} / {s.grams}g）</span>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 13, color: '#2563EB', fontWeight: 800 }}>{s.carbs}g</span>
                    <button onClick={() => removeStaple(s.key)} style={{ background: 'none', border: 'none', color: '#EF4444', cursor: 'pointer', fontSize: 16, lineHeight: 1 }}>×</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          {STAPLE_OPTIONS.map(opt => {
            const selected = selectedStaples.find(s => s.key === opt.key);
            return (
              <div key={opt.key} style={{
                background: '#fff', border: `1.5px solid ${selected ? '#BFDBFE' : '#E2E8F0'}`,
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span style={{ fontSize: 14, fontWeight: 700, color: '#1E293B' }}>{opt.label}</span>
                    <span style={{ fontSize: 10, color: '#94A3B8' }}>×{opt.ratio * 100}%</span>
                  </div>
                  {selected && (
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {opt.choices.map(choice => {
                    const carbs = Math.round(choice.grams * opt.ratio);
                    const isActive = selected?.choiceLabel === choice.label;
                    return (
                      <button
                        key={choice.label}
                        onClick={() => selectStaple(opt, choice)}
                        style={{
                      ...
                      >
                        <span style={{ fontSize: 12, fontWeight: 700 }}>{choice.label}</span>
                        <span style={{ fontSize: 10, color: isActive ? '#3B82F6' : '#94A3B8' }}>
                          {choice.grams}g → 炭水化物 {carbs}g
                        </span>
                      </button>
                    );
                  })}
